...
def main():
    ...
    lamda = None
    NLT = NonlinearTrans(data.dim, lamda) 

    NLT.train(data.train_X, data.train_Y) # train w/o regularization
    print("Linear Regression after NonlinearTrans")
    print("Ein: %f, Eout: %f" % (NLT.calc_error(data.train_X, data.train_Y), NLT.calc_error(data.test_X, data.test_Y)))

    rwd_k = np.arange(-3, 4)
    for k in rwd_k:
        lamda = math.pow(10.0, k)
        NLT.change_lambda(lamda)
        NLT.train_regularization(data.train_X, data.train_Y)
        print("Decay factor = %d," % k) # train with regularization
        print("Ein: %f, Eout: %f" % (NLT.calc_error(data.train_X, data.train_Y), NLT.calc_error(data.test_X, data.test_Y)))
